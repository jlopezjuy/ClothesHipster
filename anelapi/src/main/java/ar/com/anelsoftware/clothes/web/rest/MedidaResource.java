...
public class MedidaResource {
    ...
    /**
     * SEARCH  /_search/medidas?query=:query : search for the medida corresponding
     * to the query.
     *
     * @param query the query of the medida search 
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/medidas")
    @Timed
    public ResponseEntity<List<MedidaDTO>> searchMedidas(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of Medidas for query {}", query);
        Page<MedidaDTO> page = medidaService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/medidas");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
